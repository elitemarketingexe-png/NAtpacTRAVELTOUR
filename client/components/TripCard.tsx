...
export function TripCard({ trip, index }: { trip: TripRecord; index: number }) {
  ...
  const durationMin = Math.max(
    1,
    Math.round((trip.endedAt - trip.startedAt) / 60000),
  );
  return (
    <motion.article
      layout
          ...
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              ...
              <div className="text-xs text-muted-foreground truncate">
                {fmt(trip.startedAt)} → {fmt(trip.endedAt)} • {trip.path.length}{" "}
                pts • {durationMin} min{distance ? ` • ${distance}` : ""}
              </div>
              ...
              {trip.pois && (
                <div className="mt-0.5 flex flex-wrap gap-1.5">
                  {trip.pois.busStops.slice(0, 2).map((n, i) => (
                    <span
                      key={`b-${i}`}
                      className="rounded bg-zinc-500/10 px-1.5 py-0.5 text-[10px] text-zinc-600 dark:text-zinc-300"
                    >
                      {n}
                    </span>
                  ))}
                  {trip.pois.metro.slice(0, 1).map((n, i) => (
                    <span
                      key={`m-${i}`}
                      className="rounded bg-zinc-500/10 px-1.5 py-0.5 text-[10px] text-zinc-600 dark:text-zinc-300"
                    >
                  ...
                  {trip.pois.attractions.slice(0, 2).map((n, i) => (
                    <span
                      key={`a-${i}`}
                      className="rounded bg-zinc-500/10 px-1.5 py-0.5 text-[10px] text-zinc-600 dark:text-zinc-300"
                    >
