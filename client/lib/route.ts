...
export type RouteResult = { distanceKm: number; durationMin: number; coords: [number, number][]; steps?: string[] };

export async function fetchRoute(start: LatLng, dest: LatLng, opts?: { steps?: boolean; profile?: 'foot'|'driving'|'cycling' }): Promise<RouteResult | null> {
  try {
