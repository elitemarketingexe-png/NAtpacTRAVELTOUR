...
export default function AR() {
  ...
  const nearest = useMemo(() => {
    ...
    const best = allStops
      .map((s) => ({ stop: s, dist: haversine(pos.lat, pos.lng, s.position.lat, s.position.lng) }))
          ...
          <div className="mx-auto h-48 w-48 rounded-full border-4 border-primary/30 grid place-items-center relative overflow-hidden">
            <div className="absolute inset-0 grid place-items-center">
              <div
                ...
                style={{ transform: `rotate(${heading}deg)`, transformOrigin: "bottom center" }}
              />
            </div>
            <div className="text-xs text-muted-foreground">Point your phone towards the arrow</div>
          </div>
          {nearest ? (
            <div className="text-sm">Nearest stop: <span className="font-semibold">{nearest.stop.name}</span> • {(nearest.dist/1000).toFixed(2)} km</div>
          ) : (
            <div className="text-sm text-muted-foreground">Enable location to find the nearest stop</div>
          )}
          <Button onClick={() => navigator.geolocation?.getCurrentPosition(() => {}, () => {}, { enableHighAccuracy: true })}>Refresh location</Button>
        </Card>
      </section>
    </Layout>
  );
