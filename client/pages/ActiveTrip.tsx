  ...
  useEffect(() => {
    ...
    const watch = navigator.geolocation?.watchPosition(
      (p) => {
        ...
        setCoords((prev) =>
          prev.length &&
          prev[prev.length - 1][0] === c[0] &&
          prev[prev.length - 1][1] === c[1]
            ? prev
            : [...prev, c],
        );
      },
  ...
  return {
    ...
    start:
      params.get("slat") && params.get("slng")
        ? ([Number(params.get("slat")), Number(params.get("slng"))] as [
            number,
            number,
          ])
        : undefined,
    dest:
      params.get("dlat") && params.get("dlng")
        ? ([Number(params.get("dlat")), Number(params.get("dlng"))] as [
            number,
            number,
          ])
        : undefined,
    ...
    cd: params.get("cd")
      ? (() => {
          try {
            return JSON.parse(decodeURIComponent(params.get("cd")!));
          } catch {
            return [];
          }
        })()
...
export default function ActiveTrip() {
  ...
  const completeTrip = async () => {
    ...
    const record = saveTrip({
      ...
      start: q.start
        ? { lat: q.start[0], lng: q.start[1] }
        : { lat: center[0], lng: center[1] },
      destination: q.dest
        ? { name: q.dname, position: { lat: q.dest[0], lng: q.dest[1] } }
        : { position: { lat: end[0], lng: end[1] } },
      path,
      stops,
      pois: suggestions ?? pois,
      companionsDetails: (q.cd as any) ?? [],
      distanceKm: q.dist,
      costEstimate: q.est,
      costActual: q.cost,
    } as any);
    ...
    <Layout>
      <section className="mx-auto w-full max-w-6xl px-4 py-4 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
              <div>
                ...
                <ul className="list-disc pl-4 text-xs">
                  {suggestions.busStops.slice(0, 5).map((n, i) => (
                    <li key={i}>{n}</li>
                  ))}
                </ul>
              </div>
              <div>
                ...
                <ul className="list-disc pl-4 text-xs">
                  {suggestions.metro.slice(0, 3).map((n, i) => (
                    <li key={i}>{n}</li>
                  ))}
                </ul>
              </div>
              <div>
                ...
                <ul className="list-disc pl-4 text-xs">
                  {suggestions.attractions.slice(0, 5).map((n, i) => (
                    <li key={i}>{n}</li>
                  ))}
                </ul>
              </div>
        ...
        <Card className="p-3 grid grid-cols-4 gap-2 items-center">
          ...
          <Button
            variant={batterySave ? "default" : "secondary"}
            onClick={() => setBatterySave((v) => !v)}
          >
