...
export default function TripDetail() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const visitedBefore = trips.some(
        (t) =>
          t.tripId !== trip.tripId &&
          (t.destination?.name || "").toLowerCase() === destName &&
          t.endedAt < trip.endedAt,
      );
      if (!visitedBefore && trip.start && trip.destination?.position) {
        const r = await fetchRoute(trip.start, trip.destination.position, {
          steps: true,
          profile: "foot",
        });
    ...
    return (
      <Layout>
        <section className="mx-auto w-full max-w-6xl px-4 py-6">
          <Card className="p-4">Trip not found.</Card>
        </section>
      </Layout>
    );

  const pathCoords = (trip.path || []).map((p) => [p.lat, p.lng]) as [
    number,
    number,
  ][];
  const center =
    pathCoords[0] || ([trip.start.lat, trip.start.lng] as [number, number]);
        ...
        <Card className="p-3 text-sm">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
            ...
            <div>
              ...
              <div className="font-medium">{trip.mode}</div>
            </div>
            <div>
        ...
        <Card className="p-3">
          <div className="h-[60vh] w-full overflow-hidden rounded-lg">
            ...
            >
              ...
              {suggest?.coords && (
                <Polyline
                  positions={suggest.coords as any}
                  pathOptions={{
                    color: "#93c5fd",
                    weight: 4,
                    dashArray: "6 6",
              ...
              <Marker
                position={[trip.start.lat, trip.start.lng] as any}
                icon={L.divIcon({ className: "pulse-marker-start" })}
              />
              {trip.destination?.position && (
                <Marker
                  position={
                    [
                      trip.destination.position.lat,
                      trip.destination.position.lng,
                    ] as any
                  }
        ...
        {suggest?.steps && (
          <Card className="p-3 text-sm">
            <div className="font-medium mb-2">
              Suggested directions (first visit)
            </div>
