...
export default function Trips() {
  ...
  const trips = useMemo(() => {
    ...
    if (q.trim()) {
      ...
      arr = arr.filter(
        (t) =>
          ...
          (t.destination?.name || "").toLowerCase().includes(s) ||
    ...
    if (sort === "distance")
      arr.sort((a, b) => (b.distanceKm || 0) - (a.distanceKm || 0));
    return arr;
  ...
  const totals = useMemo(() => {
    ...
    const totalMin = allTrips.reduce(
      (s, t) => s + Math.max(1, Math.round((t.endedAt - t.startedAt) / 60000)),
      0,
    );
  ...
  return (
    <Layout>
      <section
        className="mx-auto w-full max-w-6xl px-4 py-4 space-y-4"
        data-loc="client/pages/Trips.tsx:21:7"
      >
        <MotionConfig transition={{ duration: 0.5, ease: [0.22, 1, 0.36, 1] }}>
            ...
            <div className="flex flex-wrap items-center gap-2 text-sm">
              <div className="relative">
                ...
                <Input
                  ...
                  placeholder="Search purpose, destination, or ID"
              ...
              <button
                className="inline-flex items-center gap-1 rounded-md border px-3 py-1 hover:bg-accent hover:text-accent-foreground"
                onClick={exportCSV}
              >
          ...
          >
            ...
            <Card className="p-3 text-sm">
              ...
              <motion.div
                key={Math.round(totals.totalMin)}
                initial={{ scale: 0.9, opacity: 0.6 }}
                animate={{ scale: 1, opacity: 1 }}
                className="text-xl font-semibold"
              >
            ...
            >
              <Card className="p-6 text-sm flex items-center justify-between">
                <div className="space-y-1">
                  ...
                  <div className="text-xs text-muted-foreground">
                    Start one from Home and it will appear here.
                  </div>
                </div>
